...
Research advancement QA: the local Research Center preview shows the new `// PUBLIC_GITHUB_ACTIVITY` panel with `STATUS: PUBLIC EVENTS LOADED`, read-only GitHub repository links, and a safe fallback explanation. The research divisions expose ALL plus four accessible category filter buttons, and the publication cards show sanitized/authorized/coordinated trust metadata. The existing hero, independent-course disclaimer, payment sections and safe simulated dashboard remain intact.

Interactive filter QA: selecting the `AI & LLM` division filter set `aria-pressed="true"`, left the `ai-llm` card visible, and hid the `web-api`, `os-tooling`, and `adversary` cards. The filter behavior is client-side only and performs no network or target action.
