...
RATAN AI के लिए public page `ai/index.html` और chat route `ai/chat/index.html` बनाए गए हैं। Public page में v1 Gemini 1.5, v2 Gemini 2.0, v3 Gemini 2.5, v4 Gemini 3.6 और v5 Gemini 3.7 Flash की पूरी version history है। v5 को **LIVE NOW** mark किया गया है। RATAN AI Explorer अब free open access है; email, access code, payment या manual approval की आवश्यकता नहीं है। Chat route noindex है और उसमें responsible-use notice तथा Gemini 3.7 Flash chat composer है।

Gemini API key को static frontend में नहीं रखा गया। `cloudflare/worker.js` में server-side proxy तैयार है। Worker अब केवल `GEMINI_API_KEY` secret अपेक्षित करता है, unconfigured deployment पर 503 देता है, message length सीमित करता है, और Gemini Interactions API के stable model string `gemini-3.7-flash` को call करता है। Lawful-use system instruction और server-side key protection बरकरार हैं।

अभी live chat को production में स्वीकार करने से पहले Cloudflare Worker deploy करना और `GEMINI_API_KEY` configure करना बाकी है। RATAN AI chat में कोई email या access-code gate नहीं है। GitHub Pages custom `/api/ai/chat` reverse proxy नहीं देता, इसलिए या तो Worker URL को `window.RATAN_AI_PROXY_URL` में configure करना होगा, या custom domain/reverse proxy के पीछे `/api/ai/chat` map करना होगा।
