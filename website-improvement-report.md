...
RATAN AI के लिए public page `ai/index.html` और chat route `ai/chat/index.html` बनाए गए हैं। Public page में v1 Gemini 1.5, v2 Gemini 2.0, v3 Gemini 2.5, v4 Gemini 3.6 और v5 Gemini 3.7 Flash की पूरी version history है। v5 को **LIVE NOW** mark किया गया है। RATAN AI Explorer अब five-minute free trial देता है। Trial के बाद existing UPI/INR और USDT ERC-20 payment page से manual confirmation और optional activation code workflow चलता है। Chat route noindex है और उसमें responsible-use notice, server-checked countdown तथा Gemini 3.7 Flash chat composer है।

Gemini API key को static frontend में नहीं रखा गया। `cloudflare/worker.js` में server-side proxy और पांच-minute trial endpoints तैयार हैं। Worker `GEMINI_API_KEY` secret तथा `TRIAL_KV` binding का उपयोग करता है, unconfigured deployment पर 503 देता है, message length सीमित करता है, trial expiry के बाद 402 gate देता है, और Gemini Interactions API के stable model string `gemini-3.7-flash` को call करता है। Lawful-use system instruction और server-side key protection बरकरार हैं।

Production में यह feature चलाने के लिए Cloudflare Worker deploy करना, `GEMINI_API_KEY` configure करना और `/api/ai/chat` तथा `/api/ai/trial` को Worker से map करना आवश्यक है। GitHub Pages अपने-आप custom `/api/ai/*` reverse proxy नहीं देता; Worker को अपने `workers.dev` URL पर चलाकर frontend में `window.RATAN_AI_PROXY_URL`/`window.RATAN_AI_TRIAL_URL` configure करना होगा, या custom domain/reverse proxy लगाना होगा। KV namespace `RATAN_AI_TRIALS` बनाया गया है और `cloudflare/wrangler.toml` में `TRIAL_KV` binding दर्ज है।
